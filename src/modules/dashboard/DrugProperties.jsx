...
export const DrugProperties = (
    {drug}) => {
        if (!drug) {
            return (
                <Box>
                    <Paper elevation={2} sx={{ justifyContent: 'center', alignItems: 'center', width: 300, height: 200, display: 'flex', flexGrow: 1}}>
        ...
        return (
            <Box>
                <Paper elevation={2} sx={{  width: 450, height: 251, overflowY: 'auto', marginBottom: '50px'}}>
                    <CardContent>
                        <Typography variant="h5" sx={{color:'#383874', 'marginBottom': '20px'}} gutterBottom component="div">Drug Properties</Typography>
                                ...
                                keys.map(key => (
                                    <Fragment key={key.title}>
                                        ...
                                        <Grid item xs={3} sx={{marginBottom : '10px'}}>
                                            <Typography sx={{ color: '#9292C1'}}>{fetchFromObject(drug, key.key)}</Typography>
                                        </Grid>
                                    </Fragment>
                                ))
                            }
